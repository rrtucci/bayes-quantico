...
For example, $J,J_z$ 
eigenvalues will be denoted by $j, j_z$.
}
\beq
...
weight vectors  of the fundamental rep (just derived).
As expected, the difference between
any two weight vectors, is a root vector.
...
There is a
one-to-one map
between the the irreps of  $\ger{su}(3)$ irreps and the WDs
of $\ger{su}(3)$.
