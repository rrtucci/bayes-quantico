...
$SYD(n_b)=$ set of SYD with $n_b$ boxes. $YT=\cup_{n_n=1}^\infty YT(n_b)$.
...
\beq
irreps(U(n)) =\cup_{n_b\leq n} \cup_{NR\leq n}STY(n_b, NR) 
\eeq
...
\beq
irreps(SU(n)) =
\cup_{n_b\leq n} \cup_{NR\leq n-1}STY(n_b, NR) 
\eeq
\end{enumerate}
\end{claim}
\proof
...
The Clebsch-Gordan series  for a rep of $S_{n_b}$
is
\beq
1 = \sum_{\caly\in S} P_\caly
\eeq
for some set $S$.

The Clebsch-Gordan series 
for a rep of $U(n)$ or $SU(n)$ is

\beq
1 = \sum_{\caly\in S}
P_\caly
\label{eq-cd-su-u-n_b}
\eeq
for some set $S$,
where each $P_\caly$
...
\eeq
The projection operators $P_{\caly_\alp}$ 
for $\caly_\alp\in STY(\caly)$
are complete and orthogonal.

Examples of Eq.(\ref{eq-cd-su-u-n_b}) are
...
\beq
irreps(U(n)) =
\cup_{n_b\leq n} \cup_{NR\leq n}STY(n_b, NR) 
\eeq
