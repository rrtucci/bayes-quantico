\chapter{Lagrange's Theorem and Orbit-Stabilizer Theorem}
\label{ch-lagranges-theorem}


\section{Lagrange's Theorem}

Let $H$ be a subgroup of a finite
group $G$ (i.e., $H\le G$)



$g H = \{gh| h\in H\}=$ {\bf left coset} with coset representative $g$


$G/H=\{gH| g\in G\}=\{gH| g\in G:H\}$

$G:H=$ set of {\bf coset representatives}



The {\bf Lagrange table} is a table with
rows labelled by $g\in G:H$, columns
labelled by $h\in H$, and table entries equal to $gh$.

\begin{claim}Every element of $G$ appears exactly once in the Lagrange Table. Therefore\footnote{Eq.(\ref{eq-finite-lagrange}) is for finite groups. For infinite Lie groups,
the order of each group
is replace by the dimension of the Lie algebra vector space: $dim(\ger{g})=dim(\ger{g}/\ger{h}) \; dim (\ger{h})$,
where $\ger{g}=\ger{g}/\ger{h}+\ger{h}$}

\beq
|G|=|G:H| \; |H|
\label{eq-finite-lagrange}
\eeq
\end{claim}
\proof

If $x\in G$,
then
$x\in xH=gH$ for some $g\in G:H$.  So every $x\in G$ appears at least in one coset.

Suppose $x$ appears in 2 cosets $g_1H$ and $g_2H$.
\beq
 g_1h_1=g_2h_2\eeq
so
\beq
g_2^{-1} g_1= h_2 h_1^{-1}\in H
\eeq
Hence, 
\beq
g_2^{-1} g_1 H = H\implies g_2H = g_1H
\eeq
\qed

A group $G$ is {\bf abelian} if $g_1 g_2=g_2 g_1$ for all $g_1,g_2\in  G$. 

$H$ is  a {\bf normal}
subgroup of $G$ (i.e., $H\trianglelefteq G$)
iff $gH=Hg$ (i.e., $gHg^{-1}=H$)
for all $g\in G$.
All subgroups  $H$ of an abelian group $G$  are
normal subgroups.

$G=$ all symmetries.

$H=$ symmetries for a given reference state

$G:H=$ a set of reference states that yield all distinct left cosets,
often called the {\bf index set}

$G/H=$ set of left cosets, often called a
{\bf quotient space or coset space}. Also called {\bf quotient group or coset group} if $H$ is a normal
subgroup



Examples:
\begin{enumerate}
\hrule
\item ($H$ not normal subgroup)




$
G=S_3 = \{e,\ (12),\ (13),\ (23),\ (123),\ (132)\}
$ The symmetric group on 3 letters,


$H = \{e,(12)\}$

Cosets

\beq\left\{
\begin{array}{ll}
eH &= \{e,(12)\}
\\
(13)H &= \{(13),(123)\}
\\
(23)H &= \{(23),(132)\}
\end{array}
\right.
\eeq

$G:H =
\{e,\ (13),\ (23)\}$ coset representatives. 

\beq
|G|= 6, \quad |G:H|=3, \quad |H|=2
\eeq





In this example, $H$ is not a normal subgroup of $G$
because 
\beq
(13)H = \{(13),\underbrace{(13)(12)}
_{(123)}\}
\eeq

and

\beq
H(13)
=\{(13),\underbrace{(12)(13)}
_{(213)}\}
\eeq
are not equal.

\hrule
\item ($H$ normal because $G$ is abelian)


$G= Z_6=\{0,1,2,3,4,5\}$
 under addition mod 6.

$H=\{0,3\}$

$G$ is abelian so every subgroup of $G$ is normal

$G:H=\{0,1,2\}$, coset representatives

Cosets

\beq
\left\{
\begin{array}{ll}
0+ H=\{0,3\}
\\
1+ H=\{1,4\}
\\
2+ H=\{2,5\}
\end{array}
\right.
\eeq

\hrule
\item ($H$ is normal subgroup even though $G$ is not abelian)

$S_3^e = \{e,(123),(132)\}$, even number of cycles

$S_3^o=\{(12),(13),(23)\}$, odd number of cycles

$G=S_3=S_3^o \cup S_3^e$



$
H = S_3^e.
$



$S_3^e \trianglelefteq S_3$ because $\s S_3^e =S_3^e \s =S_3^o$
for all $\s \in S_3$

$G:H=\{e, (12)\}$ coset representatives


Cosets

\beq
\left\{
\begin{array}{ll}
eH &= S_3^e
\\
(12)H&= S_3^o
\end{array}
\right.
\eeq

\beq
S_3 / S_3^e \cong  Z_2
\eeq

\end{enumerate}

\section{Orbit-Stabilizer Theorem}


Let a group $G$ act on a set $X$ by an action $\odot$. This means that $\odot: G\times X\rarrow X$ and $(g,x)\mapsto g\odot x$. Usually the {\bf action} $\odot$ is {\bf group conjugation}. $g\odot x= g x g^{-1}$. In the special case  of Lagrange's theorem, $X=G$ and $g\odot x=gx$.

$Stab(x)
=\{ g \in G | g\odot x = x \}
$= {\bf Stabilizer of $x$}, the symmetries of $x$, group operations that keep $x$ fixed or \qt{stabilized}.

$O(x)=\{g\odot x| g\in G \}=$ {\bf Orbit of $x$}, all possible projections of $x$ via $g\in G$.

Fig.\ref{fig-orbit-stab} represents  $O(x)$ and $Stab(x)$ pictorially.

\begin{figure}[h!]
\centering
\includegraphics[width=4in]
{lagranges-theorem/orbit-stab.png}
\caption{Pictorial representation of $O(x)$ and $Stab(x)$}
\label{fig-orbit-stab}
\end{figure}

\begin{claim}(Orbit-Stabilizer theorem for single point $x\in X$)
\beq
|G| = |O(x)| \;|Stab(x)|
\eeq
\end{claim}
\proof
\qed

$Z(G)=\{z\in G| gzg^{-1}=z \text{ for all }g\in G\}=$ {\bf center of $G$}

Suppose $H$ is a subgroup of $G$ (i.e., $H\leq G$) and the action is group conjugation.
There are two ways of generalizing $Stab(x)$, to define it
 for a subgroup $H$, instead of a 
single point $x$

\beq
\left\{
\begin{array}{l}
\cup_{h\in H}Stab(h)=
\cup_{h\in H}
\{g\in G| ghg^{-1}=h\}
=C_G(H)\quad
\begin{array}{l}\text{\bf centralizer} 
\\  \text{\bf(a.k.a. commutant)}
\end{array}
\\
Stab(H)=\{g\in G| gHg^{-1}=H\})=N_G(H)\quad \text{\bf normalizer}
\end{array}
\right.
\eeq


Both $C_G(H)$ and $N_G(H)$ are subgroups of $G$. 
Furthermore, $C_G(H)$ is a subgroup of $N_G(H)$
so

 \beq
Z(G)
\leq C_G(H)\leq N_G(H)\leq G
\eeq

$C_G(H)$ contains elements of $G$ that commute
with every element of $H$. That's why commutant
is a good name for it. The  elements of
a normalizer, on the other hand, may not commute with some  elements of $H$.

 $N_G(H)$ is the biggest subset of $G$ that leaves the subgroup $H$ fixed under conjugation.
 If $H$ is a normal subgroup of $G$, 
then $N_G(H)=G$.



$O(H)=\cup_{h\in H}O(h)=$ projections (orbits) of 
structure(subgroup) $H$ instead of single point $H$


\begin{claim}(Orbit-Stabilizer theorem for subgroup $H\leq G$)
\beq
|G| = |O(H)| \;|\underbrace{Stab(H)}_{N_G(H)}|
\eeq
\end{claim}
\proof
\qed

Suppose $G$ is a group and $H$ is a  subgroup of $G$.

For any two $a,b\in G$, define $a\sim b$ if $a=gbg^{-1}$ for some $g\in G$. This is an
equivalence relation that partitions  $G$
into disjoint classes. Now define 

$CC(x)=\{gxg^{-1}|g\in G\}=$ {\bf conjugacy class (CC) of $x\in G$}.

$CC(H)=\{gHg^{-1}|g\in G\}=
\bigcup_{h\in H}CC(h)=$ {\bf union of CCs of $H\leq G$}.

The orbit (for conjugacy action) and the conjugacy class
are the same thing. 


\beq
CC(x)= O(x)= G:Stab(x)
\eeq

\beq CC(H)= O(H)=G:Stab(H)
\eeq

A normal subgroup $H$ of $G$ satisfies $gHg^{-1}=H$
for all $g\in G$, so it equals a union of CCs.

\beq
H=\bigcup_{x\in X}CC(x)=CC(X)
\eeq
where $X\subset H$.
So conjugacy classes are the "atoms" from which normal subgroups are built.

A {\bf central conjugacy class}
is a $CC(x)$ such that $CC(x)\subset Z(G)$.
Hence, we must have  $CC(x)=\{x\}$. So central CCs are precisely the singleton sets
$\{x\}$ for all  $x\in Z(G)$.
\begin{claim} (sum of class orders)
\beq
|G|= |Z(G)| +\sum_{x\in X}
\underbrace{|G: Stab(x)|}_{|O(x)|}  \eeq
where $X$ contains representatives
of   non-central CCs (i.e., CCs with more than one element)
\end{claim}
\proof
\qed




\hrule
Examples:
\begin{enumerate}
\item 
$G= S_3$


$x=(12)$

$Stab(x)=\{e, (12)\}$

$O(x)= \{ (12), (13)(12)(13), (23)(12)(23)\} =
\{
(12), (23), (13)
\}$

\beq
|G|=6, |O(x)|=3, |Stab(x)|=2\implies
|G| = |O(x)| \;|Stab(x)|
\eeq
\hrule 
\item (sum of class orders)


$G=S_3$

conjugacy classes:

$CC_0=\{e\}$, \quad (0 cycles)

$CC_1=\{(12),(23),(13)\}$, \quad (1 cycle)


$CC_2=\{(123),(132)\}$, \quad (2 cycles)

\beq
|G|=6, |CC_0|=1,|CC_1|=3, |CC_2|=2
\implies |G|= |CC_0| + |CC_1| + |CC_2|
\eeq

\beq
Z(G)= \{e\}
\eeq

\beq
Stab((12))=\{e, (12)\}
\implies |S_3:Stab((12))|=6/2=3
\eeq

\beq
Stab((123))=\{(12), (23), (13)\}
\implies |S_3:Stab((123))|=6/3=2
\eeq

\end{enumerate}

\hrule
Applications:
\begin{itemize}
\item Quantum Error Correction

Pauli group =$\{1, \s_x, \s_y, \s_z\}\times\{\pm 1, \pm i\}$. Has 16 elements.

Clifford group
=group generated by $H=\frac{1}{\sqrt{2}}\begin{pmatrix}1&1\\1&-1\end{pmatrix}$
and $S=\begin{pmatrix}1&0\\0&i
\end{pmatrix}$. Has 24 elements

$G=$ Pauli or Clifford group

$x$= code state

$O(x)$= orbit (i.e., projections) of code state $x$

$Stab(x)=$ subgroup of $G$ that preserves (doesn't change, stabilizes, fixes) code state $x$

$H=$ code space, subgroup of $G$

$Stab(H)=$ subgroup of $G$
that fixes code space $H$


\item Thermodynamics

$G$= group

$x$= single microstate


$O(x)$= set of equivalent microstates=single macrostate

$\ln |O(x)|$= entropy

$Stab(x)=$ subgroup of $G$ that preserves (doesn't change, stabilizes, fixes) microstate $x$, 
residual symmetry group



\item Lie groups

$G$= Lie group

$H$= stabilizer, principal fibre

$G:H=$ base space


$\pi(g)=gH=$ projection, fibre

$\pi: G\rarrow G/H=\{gH: g\in G\}$ fibre bundle

Example

$G=SU(2)$

$H=U(1)$ fibre

$G:H=S^2$= two sphere, base space




\end{itemize}
