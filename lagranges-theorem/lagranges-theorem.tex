...
is replace by the dimension of the Lie algebra vector space: $dim(\ger{g})=dim(\ger{a}) \; dim (\ger{h})$,
where $\ger{g}=\ger{a}+\ger{h}$}
...
\\
Stab(H)=\{g\in G| gHg^{-1}=H\})=N_G(H)& \text{\bf Normalizer}
\end{array}
\right.
\eeq
