...
\beq
E_-=\frac{1}{\sqrt 2}\ket{2}\bra{1} , \quad E_+=\frac{1}{\sqrt 2}\ket{1}\bra{2},
...
\label{eq-xi-linear-comb}
\eeq
\begin{claim}
If we assume the normalization of $\ger{su}(2)$
operators used in Eq.(\ref{eq-su2-norm-op}), then
\beq
\xi_{i_-}=\frac{\alp_{i_-}}{\vec{\alp}\cdot \vec{\alp}}
\eeq
\end{claim}
\proof

Applying $[E_\valp,\cdot]$ to both sides of Eq.(\ref{eq-xi-linear-comb})
\beq
[E_\valp, [E_\valp, E_{-\valp}]]
=-\vec{\xi}\cdot\valp E_{\valp}
\eeq
But for $\ger{su}(2)$
\beq
[E_+, [E_+, E_-]]=
