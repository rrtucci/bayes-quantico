...
In 1+3 dimensional relativistic quantum field theory, the {\bf chirality operator} 
...
Generalize $\gamma_5 \rarrow \Gamma_5$, from $SO(1,3)$ to
$SO(p,q)$.
\beqa
\gfive &=&
\frac{1}{\sqrt{n!}}
...
\item For $n$ odd, $\Gamma_5$ is a constant. 

The Lorentz group for odd $n$ has only one irreducible spinor representation.
For even $n$, it has two inequivalent spinor irreps (positive and negative chirality).

Thus chirality does not exist in odd dimensions because the representation theory of the Lorentz group does not support it.

\item $\Gamma_5$ anticommutes  with $\gamma_\mu$ if $n$ is even. Commutes 
...
Spinor Type & Condition & \# of dofs in 4D & Exists in 4D? \\ \hline
Dirac & none & 8 & yes \\ \hline
Weyl & $\gfive \psi = \pm \psi$ & 4 & yes \\ \hline
Majorana & $\psi^c=\psi$ & 4 & yes \\ \hline
Majorana-Weyl & both & 0 & no \\ \hline
\end{tabular}
\caption{Different types of spinors, their number of dofs (real degrees of freedom) in 4D and whether they exist in 4D.}
\label{tab-maj-weyl}
\end{table}
...
The {\bf charge conjugation matrix} $C$ is defined by
...
A Dirac spinors has  4 complex components.
A Majorana spinor has 4 real components. 

The Weyl spinor and Majorana spinor constraints cannot both be imposed simultaneously in 1+3 dim.
But in some dimensions, Majorana–Weyl spinors **do** exist. (see Table \ref{tab-dim-spinor-exist}) 
...
\hline
8
& Yes, $\dim_{\mathbb R}=32$
& Yes, $\dim_{\mathbb R}=16$
& No
& No \\
\hline
\end{tabular}
\caption{Spinor types in Lorentzian signature $(1,n-1)$ (mostly minus).
Note that Majorana-Weyl spinors are possible for $n=2, 10$ (10\%8=2).
This  is why superstring theories are 10 dimensional.
}
\label{tab-dim-spinor-exist}
\end{table}
