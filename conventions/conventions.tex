...
$(y^b)=(y^1, y^2, \ldots, y^n)= \bar{y}^{:n}\in \bar{V}^n=\CC^{n\times 1}$. $V^n$ is the lower indices vector space and
$\bar{V}^n$ is its {\bf dual vector space} (i.e., with upper indices).
...
\beq
(M^\dagger)\indices{_b^a}= M\indices{^a_b},\quad
(M^T)\indices{_b^a}= (M^*)\indices{^a_b},
\eeq
Thus, $\dagger$ aand $T$ only move
the indices horizontally, to change their horizontal order. To move indices vertically, up or down,
we use the metric $g$ (i.e.,
$x^b=g^{ba}x_a$).
...
\beq
M^b_a=M\indices{^b_a}= M\indices{_a^b}
\eeq
