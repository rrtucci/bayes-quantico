...
\section{Direct Sum and Tensor Product of Matrices}\label{sec-direct-sum-prod}

The {\bf Cartesian product} $A\times B$ of two sets $A$ and $B$ (or of two groups or of two vector spaces, if they are being treated as sets) is defined as a set of ordered pairs

\beq
A\times B =\{(a,b)\mid  a\in A, b\in B\}
\eeq

Sometimes we want to indicate 
that $A\times B$ is endowed with an
operation that is bilinear. In those cases,
instead of the {\bf times symbol} $\times$,  we use the  {\bf otimes (a.k.a., tensor product) symbol} $\otimes$.
...
The {\bf Kronecker tensor product or direct product} of two rectangular matrices
$A$ and $B$ of arbitrary size is defined as the generalization of the following example
...
Besides $A\otimes B$ being used to denote the tensor product of two matrices $A$ and $B$, it is used  to denote
the tensor product of two operators or two vector spaces. In all cases,
there is a bilinear operation at play. But note that\footnote{$\cong$ means
there is a 1-1 onto map
between them.}

\beq
A\times B\cong A\otimes B
\eeq
so they are often used indistinguishably.
...
To prove this, just expand the exponentials on both sides, neglecting
 terms of order $t^3$.
...
Suppose $\rho: \calg\rarrow \CC^{n\times n}$ is a rep of group $\calg$ and $a_i, b_i, c_i,  c_i'\in \RR$.
Let
...
\begin{align}
[e^{sA}, e^{tB}] &= \left[1 + sA  + tB+ \frac{1}{2}ts[A,B]\right]-
\left[1 + sA  + tB+ \frac{1}{2}ts[B,A]\right]
+\calo(3)
\\
&=
ts[A,B] +\calo(t^is^j\mid i+j\geq 3)
\end{align}
Hence

\beq
ts[A, B] = ts(C-C') +\frac{(ts)^2}{2}\left[C^2-(C')^2]\right]
+\calo(t^is^j\mid i+j\geq 3)
\eeq
Hence, we must have

\beq
C=-C',\quad 
\underbrace{[A,B]}_{a_i b_j [X_i, X_j]}= 2\underbrace{C}_{c_k X_k}
\eeq
...
acts like a derivative operator on product $[Y, Z]$.
...
$L(V)=End(V)$ = the {\bf endomorphisms of $V$}, i.e., the set of all linear maps
from $V$ to $V$, 
...
\qt{acts on} the vector space $V$. One  way to specify a representation is
...
If the map $\rho: \calg\rarrow C^{n\times n}$
...
Let $L(V)$ denote the set of all
linear  operators on a vector space $V$.
In this section, we will
assume the group $\calg$ and
its Lie algebra $\ger{g}$ are subsets of $L(V)$.

Note that

\beq
\exp: \ger{g}\rarrow \calg, \quad
\exp(X) =g
\eeq

\beq
D: \calg \rarrow \ger{g}, \quad
\underbrace{Dg}_{X} = \lim_{t\rarrow 0}\frac{d}{dt}
\underbrace{g(t)}_{e^{tX}}
 \eeq
($Dg =\ln g$ also)

Define the adjoint  function $ad$ by

\beq
ad: \ger{g}\times L(V)\rarrow L(V), \quad
\adj{X}(Y)=[X, Y]
\eeq
Note that $\adj{X}$ can be thought of as
half a commutator bracket:
...
We can also define the Adjoint (with capital \qt{A}) function 
$Ad$ by

\beq
Ad: \calg\times L(V) \rarrow L(V),
\quad \Adj{g}(h)= g h g^{-1}
\eeq
...
\beqa
\left[\frac{d}{d\lam}\Adj{e^{\lam X}}(Y)\right]_{\lam=0}
