...
The {\bf trivial \qt{representation}}
is the rep with  $\phi(g)=1=[1]^{1\times 1}$ 
for all $g\in \calg$.
The dimension 
of this \qt{rep} is $d_\lam=1$.
This \qt{rep} is often not
considered a rep because it
doesn't capture any of the group structure.

A {\bf singlet or scalar representation}
is any 1 dimensional rep,
including the trivial rep.\footnote{Some books use the terms \qt{singlet rep} and \qt{scalar rep} more narrowly to refer to the trivial rep only. Cvitanovic's birdtracks book \cite{birdtracks-book} and this book don't.} For example,
the rep with  $\phi(g)=e^{i\beta(g)}=[e^{i\beta(g)}]^{1\times 1}$ 
for all $g\in \calg$,
where $\beta(g)\in \RR$.
The dimension 
of this rep is $d_\lam=1$.
If $\phi(g)=e^{i\beta(g)}diag(1,1)$,
this is sometimes refered to
as a singlet
rep too, or two identical copies of a singlet rep.
...
the term \qt{dimension} 
by itself for case 1.
For case 2, we will
add the adjective \qt{matrix}.
