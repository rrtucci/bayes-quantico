...
operation that is bilinear. In those cases we use the symbol $\otimes$ which is called the {\bf otimes or tensor product or direct product operator}.
...
\begin{align}
[e^{tA}, e^{sB}] &= \left[1 + tA  + sB+ \frac{1}{2}ts[A,B]\right]-
\left[1 + tA  + sB+ \frac{1}{2}ts[B,A]\right]
+\calo(3)
\\
&=
...
Define the adjoint  function $\adj{X}$ by

\beq
\adj{X}: \ger{g}\rarrow L(V), \quad
\adj{X}[(Y)=[X, Y]
\eeq
...
\beq
\Adj{g}: \calg\rarrow L(V),
\quad \Adj{g}(Y)= g Y g^{-1}
\eeq

Note that $\adj{X}$ and $\Adj{g}$
