...
of a group $\calg$}
is a map $\rho: \calg\rarrow \CC^{n\times n}$\footnote{More generally, the $\CC^{n\times n}$ can be replaced by $\RR^{n\times n}$ or by $\FF^{n\times n}$ for any field $\FF$} such that
...
\beq
\rho (a)\ket{i}= \sum_j M_{ij}\ket{j}
\implies \av{i|\rho(a)|j} = M_{ij}
\eeq
...
\begin{enumerate}
\item (adjoint rep MD) the number $N$ of generators $T_i$, where $i=1,2,
\ldots, N$.
\item (def rep MD)
