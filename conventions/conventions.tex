...
\section{Direct Sum and Tensor Product of Matrices}
...
there is a bilinear tensor operation at play.
...
\quad \Adj{e^X}(Y)= e^X Y e^{-X}
\eeq

Note that $\adj{X}$ and $\Adj{e^X}$
...
For any  group $\calg$,
define 2  rep operators $\Omega$ and $\Omega_{adj}$ so that
