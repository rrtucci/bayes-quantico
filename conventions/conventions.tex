...
The {\bf tensor product or direct product} of two rectangular matrices
...
there is a bilinear tensor operation at play. But note that\footnote{$\cong$ means
...
Define the adjoint  function $adj$ by

\beq
adj: \ger{g}\times l9v\rarrow L(V), \quad
\adj{X}(Y)=[X, Y]
\eeq
...
We can also define the Adjoint (capital \qt{A}) function 
$Adj$ by

\beq
Adj: \calg\times L(V) \rarrow L(V),
\quad \Adj{g}(h)= g h g^{-1}
\eeq

Note that $adj$ and $Adj$
