...
Assume $\calg, \calh_1, \calh_2$ are
groups and $\calg\supset \calh_1\supset \calh_2$.
The first BD step takes
\beq
\{\rho(g)\mid g\in \calg\}\xymatrix{\ar[r]_{BD}&}\{\rho_1(h_1)
\oplus \rho_2(h_1) \oplus \ldots \oplus \rho_n(h_1)\mid h_1\in \calh_1\}
\eeq
where $\rho()$ is an irrep of $\calg$
and $\rho_i()$ for $i=1,2, \ldots, n$
are irreps of  $\calh_1$.
$\rho_i()$ can be obtained by
block diagonalizing the restriction
$\rho\downarrow  \calh_1(h_1)$ for each $h_1\in \calh_1$.

In the second BD step,

\beq
\left\{\bigoplus_{i=1}^n \rho_i(h_1)\mid h_1\in \calh_1\right\}
\xymatrix{\ar[r]_{BD}&}
\left\{\bigoplus_{i=1}^n \bigoplus_{j=1}^m\rho_{i,j}(h_2)\mid  h\in \calh_2\right\}
\eeq
where the 
$\rho_{ij}()$ are irreps of $\calh_2$.
$\rho_{ij}()$ can be obtained by
block diagonalizing the restriction
$\rho\downarrow  \calh_2(h_2)$ for each $h_2\in \calh_2$.
In general, we can have any number of BD steps,
not just 2.
...
Define a
 {\bf tree of reps (TOR)} as a tree wherein each node represents an irrep of $\calh_j$ and arrows point
from an irrep of $\calh_j$ to a direct sum of irreps
of $\calh_{j+1}$ where $\calh_j\supset \calh_{j+1}$
and $\calh_j,  \calh_{j+1}$ are subgroups of $\calg$.
...
{\color{red}If
the root node irrep of a TOR is the adjoint representation of $\calg$, then the
dimension of each irrep in the tree is also the number of generators for that irrep.}
Hence, we will call
the TOR for the
adjoint rep of $\calg$, a {\bf tree of generators (TOG)}. For
...
between the  dots of 
a weight diagram of 
the adjoint rep of $\calg$, and the generators of $\calg$}.
...
\begin{array}{l}
\text{for all }u,\quad
[Q_u,\calh]=0, \quad Q_u\ket{vac_0}=0
\\
\text{for all } b,\quad [X_b,\calh]= 0, \quad X_b\ket{vac_0}\neq 0
\end{array}\right.
\eeq

\beq
[\ger{g},\calh]=0, \quad\ger{h}\ket{vac_0}=0,
...
\beq
\underbrace{U(1)}_\calg\supset \underbrace{\{e\}}_\calh
\eeq
...
Let $n=p+q$\footnote{Note that given two groups $\calg_1$, $\calg_2$,
their Cartesian product $\calg_1\times \calg_2$ is defined, but
not their direct sum $\calg_1\oplus \calg_2$
or direct (Kronecker) product $\calg_1\otimes \calg_2$. Direct sums and products are defined for vectors (and vector spaces) and matrices (and matrix  reps).}
...
In both cases, it's 12, so the number of  dofs is conserved. Massless gauge bosons have 2 dofs (2 polarizations) and massive ones have 3 dofs (3 polarizations). The Higgs field initially has
