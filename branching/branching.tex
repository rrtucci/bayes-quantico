...
add small perturbation $\delta \calh$ to
Hamiltonian $\calh$.
...
Each $Q_u$ corresponds to a massless gauge boson.

Each $X_b$ corresponds to a massive gauge bosoon.
...
(in electroweak symmetry breaking $\phi\in \CC^2$). $v>0$ is called the {\bf vacuum expected value (VEV)}.
This potential $V$ is plotted in Fig.\ref{fig-mex-hat}.
It's minimum is when $|\phi|=v$ and
$V(e^{i\theta}\phi)= V(\phi)$.
Consider
...
\beq
\begin{array}{ll}
\text{extremum that does violate symmetry:}& \phi = 0
\\
...
1 unbroken generator (electric charge $Q=T_3 +\frac{1}{2}Y$) yields
massles photons.

3 broken generators ($T_1, T_2, T_3- \sin(\theta_W) Q  $) yield 3 Goldstone (Go) modes (massless). The 3 Go modes become the longitudinal modes of the 3 gauge bosons $W^+, W^-, Z^0$. Massless particles like the photon have 2 transverse polarizations, and no longitudinal one.
...
\begin{tabular}{|l|l|l|}
\hline
 & \cellcolor[HTML]{BFFCAE}before VEV & \cellcolor[HTML]{BFFCAE}after VEV \\ \hline
\cellcolor[HTML]{FFCCC9}massive gauge bosons &  & 3 (9 dofs) \\ \hline
\cellcolor[HTML]{FFCCC9}massless gauge bosons & 3 (6 dofs) &  \\ \hline
\cellcolor[HTML]{FFCCC9}massive Higgs &  & 1 (1 dof) \\ \hline
\cellcolor[HTML]{FFCCC9}massless Higgs & 1 (4 dofs) &  \\ \hline
\end{tabular}
\caption{Number of degrees of freedom (dofs) before and after VEV.}
\label{tab-dof-vev}
\end{table}

Table \ref{tab-dof-vev}
gives an inventory of the number of (real)
degrees of freedom (dofs) before and after VEV.
In both cases, it's 10, so it's conserved. Massless gauge bosons have 2 dofs (2 polarizations) and massive ones have 3 dofs (3 polarizations). The Higgs field initially has
4 (real) dofs because it's a $\CC^2$ vector.
The Higgs field finally is reduced to a real number.

It might sound  paradoxical that massless Go-boson +
massless gauge boson = massive gauge boson.
This is explained intuitively by 
the analogy of when light (massless gauge boson)
goes through a medium (Go-bosons), it slows down to speeds less than speed of light.
