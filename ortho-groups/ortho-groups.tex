...
Often we will
revert to an even
less precise birdtrack notation
shown below 
wherein the arrow directions are omitted.
We will try to 
use only lower indices in all
tensors except the metric   tensor
which may have 2 upper or 2
lower indices or one
upper and one lower. In the
latter 
case, it becomes the Kronecker delta.
...
\eeq
or, equivalently,
the generator are anti-symmetric matrices:
...
Note that the anti-symmetrizer
$\cala_2$ 
is an invariant matrix because
...
Define the following 3 projection operators (PO)
\begin{enumerate}
\item Singlet PO
\beq
(P_S)\indices{_\mu_\nu^\rho^\s}
