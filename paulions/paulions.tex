...
($dim_\RR$, number of real degrees of freedom, dofs. $dim_\CC= 2dim_\RR$. For example, $dim_\RR(\HH)=4$, Where $\HH$ are
the quaternions.
...
Only the GROUP
$Spin(n)_\RR$ describes physical processes.
...
\end{tabular}
\caption{Sets isomorphic ($\cong$) to $Cl(n)_\RR$. In this figure, $M_n(\FF)=\FF^{n\times n}$ for $\FF=\HH, \RR,\CC$ where
...
\begin{itemize}
\item Dimension check: the real dimension  for the two isomorphic sets is $2^n$ in every case.  For
example, $\dim_\RR M_2(\CC) = 8 = 2^3$.
