...
U=e^{-i\frac{\theta}{2}\s_{\hat{a}}}
\eeq
and (this is called Rodrigues formula)
...
that the group $Spin(n)_\RR$ acts on.
...
$Cl(n)_\FF$ is a span over $\FF=\RR,\CC$
...
the vector space. Lie Algebras are
real vector spaces, so they can be 
complexified, although they rarely are. $Cl(n)_\CC$ is the complexification
of  $Cl(n)_\RR$.
...
A complex rep of $Spin(n)_\RR$
and $Spin(n)_\CC$
are not the same thing although
some people erroneously equate them.

The irreps of $Spin(n)_\RR$
are complicated. They can be 
real, complex and even quaternionic. Rather than depending on $n$, they depend on $n \mod 8$ (or, in Python notation, $n\% 8$,
the remainder after dividing $n$ by 8.) (Bott Periodicity).
The complex irreps of $Spin(n)_\RR$,
for those $n$ for which they exist,
such as $n=4$, are as follows:
