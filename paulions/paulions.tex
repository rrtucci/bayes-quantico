...
\hat{\theta}=
\frac{\overbrace{(\hat{a}\times\vec{x})}^{\vec{\theta}}}
{|\vec{x}|\sin\beta},\quad
\hat{r}=
\frac{\overbrace{[\vec{x}-\hat{a}(\hat{a}\cdot\vec{x})]}^{\vec{r}}}{|\vec{x}|\sin\beta}
\eeq
...
\end{tabular}
\caption{Sets isomorphic ($\cong$) to $Cl(n)_\RR$.  $M_n(\FF)=\FF^{n\times n}$ for $\FF=\HH, \RR, \CC$.
...
\eeq
Rather than depending on $n$, they depend on $n \mod 8$ (or, in Python notation, $n\% 8$,
